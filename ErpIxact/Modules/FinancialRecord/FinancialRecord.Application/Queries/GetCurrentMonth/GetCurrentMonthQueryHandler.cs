...
public class GetCurrentMonthQueryHandler
    : IRequestHandler<GetCurrentMonthQuery, Result<PagedResultDto<FinancialRecordDto>>>
{
    private readonly IFinancialRecordRepository _repository;

    public GetCurrentMonthQueryHandler(IFinancialRecordRepository repository)
    {
        _repository = repository;
    }

    public async Task<Result<PagedResultDto<FinancialRecordDto>>> Handle(
        GetCurrentMonthQuery request,
        CancellationToken cancellationToken)
    {
        var (items, totalCount) = await _repository.GetAllCurrentMonthAsync(
