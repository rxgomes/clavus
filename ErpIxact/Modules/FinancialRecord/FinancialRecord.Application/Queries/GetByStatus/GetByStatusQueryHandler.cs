...
public class GetByStatusQueryHandler
    : IRequestHandler<GetByStatusQuery, Result<PagedResultDto<FinancialRecordDto>>>
{
    private readonly IFinancialRecordRepository _repository;

    public GetByStatusQueryHandler(IFinancialRecordRepository repository)
    {
        _repository = repository;
    }

    public async Task<Result<PagedResultDto<FinancialRecordDto>>> Handle(
        GetByStatusQuery request,
        CancellationToken cancellationToken)
    {
        var (items, totalCount) = await _repository.GetByStatusAsync(
